...
UPDATE_MONODOMAIN(update_monodomain_default) {
    ...
    int recv_count = (int)initial_number_of_cells;
    ...
    if(use_gpu) {
        ...
        if(!the_ode_solver->is_parallel) {
            ...
            MPI_Gatherv(vms, recv_count, MPI_DOUBLE, vms, the_ode_solver->stim_counts, the_ode_solver->stim_displacements, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }
    }
    #endif
