...
void set_ode_initial_conditions_for_all_volumes(struct ode_solver *solver, struct string_hash_entry *ode_extra_config) {
    ...
    if(rank == 0 && solver->is_parallel){
        if(solver->sv != NULL) {
            free(solver->sv);
        }

        uint32_t num_cells = solver->original_num_cells;
        solver->sv = (real*)malloc(solver->model_data.number_of_ode_equations*num_cells*sizeof(real));
    }
    else {
...
void solve_all_volumes_odes(struct ode_solver *the_ode_solver, real_cpu cur_time, struct string_voidp_hash_entry *stim_configs,
    ...
    if(stim_configs) {
        if (rank == 0) { //mestre configura os stims
            real stim_start = 0.0;
            real  stim_dur = 0.0;
        ...
        //se distribuído, mestre faz o envio de sv e stims para os escravos
        if (the_ode_solver->is_parallel) {
            MPI_Scatterv(&merged_stims[0], the_ode_solver->stim_counts, the_ode_solver->stim_displacements, MPI_DOUBLE, &merged_stims[0], the_ode_solver->buffer_size_stim, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        }

        if(the_ode_solver->gpu) {
#ifdef COMPILE_CUDA
            if (rank != 0) { //exceto o mestre resolve as ODE's
                solve_model_ode_cpu_fn *solve_odes_pt = the_ode_solver->solve_model_ode_gpu;
                solve_odes_pt(the_ode_solver, ode_extra_config, cur_time, merged_stims);
            } else if (!the_ode_solver->is_parallel) { //sem sistema distribuído
                solve_model_ode_cpu_fn *solve_odes_pt = the_ode_solver->solve_model_ode_gpu;
                solve_odes_pt(the_ode_solver, ode_extra_config, cur_time, merged_stims);
            }
#endif
