...
COMPILE_EXECUTABLE () {
	...
	if [ -n "$FORCE_COMPILATION" ]; then
		...
		local COMPILER_COMMAND="mpicc $MY_C_FLAGS $SOURCES ${STATIC_DEPS[*]}  -o $BUILD_DIR/${EXECUTABLE_NAME} ${EXTRA_LIBRARY_PATH[*]} ${DYNAMIC_DEPS[*]} -Wl,-rpath=$LIBRARY_OUTPUT_DIRECTORY"
